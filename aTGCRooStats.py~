...
from ROOT import RooWorkspace, TFile, TH1, TChain, RooDataHist, \
     ...
     RooArgSet, RooArgList, TH2F, TH2D, TTree, TF2, RooFormulaVar, TCanvas, TProfile2D
#pretty plots stuff from Irakli
from beautify import beautify
from initCMSStyle import initCMSStyle

nGridPointsForNewF=21
...
def fitATGCExpectedYields(ws,cfg,section):
    ...
    outfile_newF = TFile.Open('input_forNewF_'+section+'_'+cfg.get('Global','couplingType')+'.root','RECREATE')
...
#        newFormatInput = TH2D('bin_content_lam_dk_'+str(i)+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV','bincontent',
 #                          nGridPointsForNewF,cfg.getfloat(section,'par1GridMin'),cfg.getfloat(section,'par1GridMax'),
 #                          nGridPointsForNewF,cfg.getfloat(section,'par2GridMin'),cfg.getfloat(section,'par2GridMax'))
 #       newFormatInput_lamdg = TH2D('bin_content_lam_dg_'+str(i)+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV','bincontent',
 #                          nGridPointsForNewF,cfg.getfloat(section,'par1GridMin'),cfg.getfloat(section,'par1GridMax'),
 #                          nGridPointsForNewF,cfg.getfloat(section,'par2GridMin'),cfg.getfloat(section,'par2GridMax'))
 #       newFormatInput_dkdg = TH2D('bin_content_dk_dg_'+str(i)+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV','bincontent',
 #                          nGridPointsForNewF,cfg.getfloat(section,'par1GridMin'),cfg.getfloat(section,'par1GridMax'),
 #                          nGridPointsForNewF,cfg.getfloat(section,'par2GridMin'),cfg.getfloat(section,'par2GridMax'))
 #       
 #       newFormatCoef = TH1F('parameters_'+str(i)+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV','bincoeff',
 #                            6,-0.5,5.5)


        newFormatInput = TH2D('bin_content_lam_dk_'+str(i)
   #                           +'_'+str(int(binMin))
   #                           +'to'+str(int(binMax))+'GeV'
                              ,'bincontent',
                          nGridPointsForNewF,cfg.getfloat(section,'par1GridMin'),cfg.getfloat(section,'par1GridMax'),
                          nGridPointsForNewF,cfg.getfloat(section,'par2GridMin'),cfg.getfloat(section,'par2GridMax')
                              )
        newFormatInput_lamdg = TH2D('bin_content_lam_dg_'+str(i)
                                    #+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV'
                                    ,'bincontent',
                           nGridPointsForNewF,cfg.getfloat(section,'par1GridMin'),cfg.getfloat(section,'par1GridMax'),
                           nGridPointsForNewF,cfg.getfloat(section,'par2GridMin'),cfg.getfloat(section,'par2GridMax'))
        newFormatInput_dkdg = TH2D('bin_content_dk_dg_'+str(i)
                                   #+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV'
                                   ,'bincontent',
                           nGridPointsForNewF,cfg.getfloat(section,'par1GridMin'),cfg.getfloat(section,'par1GridMax'),
                           nGridPointsForNewF,cfg.getfloat(section,'par2GridMin'),cfg.getfloat(section,'par2GridMax'))
        
        newFormatCoef = TH1F('parameters_'+str(i)
                             #+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV'
                             ,'bincoeff',
                             6,-0.5,5.5)
...
#            print obs_mc.GetName(),' > ',str(binMin)
        ...
        print "----> checking Vasiliki's input:"
        print "parameters: ",func.GetParameter(0),", ",func.GetParameter(1),", ",func.GetParameter(2),", ",func.GetParameter(3),", ",func.GetParameter(4),", ",func.GetParameter(5)
        newFormatCoef.SetBinContent(1,func.GetParameter(0))
        newFormatCoef.SetBinContent(2,func.GetParameter(1))
        newFormatCoef.SetBinContent(3,func.GetParameter(2))
        newFormatCoef.SetBinContent(4,func.GetParameter(3))
        newFormatCoef.SetBinContent(5,func.GetParameter(4))
        newFormatCoef.SetBinContent(6,func.GetParameter(5))

        par1_value=0.
        par2_value=0.
        SM_yield=func.GetParameter(0)+func.GetParameter(1)*par1_value+func.GetParameter(2)*par2_value+func.GetParameter(3)*par1_value*par2_value+func.GetParameter(4)*par1_value*par1_value+func.GetParameter(5)*par2_value*par2_value
        ...
        for bin_x in range(1,nGridPointsForNewF+1):
            for bin_y in range(1,nGridPointsForNewF+1):
                ...
                print newFormatInput.GetName()," -> ",bin_x,",",bin_y," par: ",par1_value,",",par2_value, " yield: ", yield_bin

        newFormatInput_lamdg=newFormatInput.Clone("name1")
        newFormatInput_lamdg.SetName('bin_content_lam_dg_'+str(i)
                                     #+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV'
                                     )
        newFormatInput_dkdg=newFormatInput.Clone("name2")
        newFormatInput_dkdg.SetName('bin_content_dk_dg_'+str(i)
                                    #+'_'+str(int(binMin))+'to'+str(int(binMax))+'GeV'
                                    )
        print "read SM yield: ", newFormatInput.GetBinContent(newFormatInput.FindBin(0.,0.))
        newFormatInput.Scale(1./SM_yield)
        print "@@@@@@@@@@@@@@@@!!!!!!!!!  after scaling: read SM yield: ", newFormatInput.GetBinContent(newFormatInput.FindBin(0.,0.))
        newFormatInput_lamdg.Scale(1./SM_yield)
        newFormatInput_dkdg.Scale(1./SM_yield)
        newFormatInput.Write()
        newFormatInput_lamdg.Write()
        newFormatInput_dkdg.Write()
#        TProfile2D&(newFormatInput_dkdg).Write()
#        newFormatInput.Write()
        newFormatCoef.Write()
